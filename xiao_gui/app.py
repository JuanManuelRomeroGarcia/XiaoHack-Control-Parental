...
try:
    from app.scheduler import is_play_allowed, check_playtime_alerts
except Exception:
    is_play_allowed = None
    check_playtime_alerts = None

try:
    import app.notifier as notifier_mod  # overlay + toasts
except Exception:
    notifier_mod = None
...
# ---------------- Versión del runtime ---------------------------------------
def _read_text_clean(p: Path) -> str:
    # Lee UTF-8 con/sin BOM y limpia
    return p.read_text(encoding="utf-8-sig").strip().lstrip("\ufeff")
...
def _read_version() -> str:
    ...
    try:
        vtxt = root / "VERSION"
        if vtxt.exists():
            v = _normalize_version(_read_text_clean(vtxt))
            if v:
                return v
    except Exception:
        pass
...
class TutorApp(tk.Tk):
    ...
    def _on_toggle_theme(self):
        ...
        self._save_cfg(self.cfg)
        try:
            apply_theme(self, "dark" if dark else "light")
